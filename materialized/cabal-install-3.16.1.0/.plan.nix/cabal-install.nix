{ system
  ...
  , ... }:
  {
    ...
    package = {
      specVersion = "3.6";
      identifier = { name = "cabal-install"; version = "3.16.1.0"; };
      ...
      dataDir = ".";
      dataFiles = [];
      extraSrcFiles = [ "bash-completion/cabal" ];
      extraTmpFiles = [];
    ...
    components = {
      ...
      exes = {
        "cabal" = {
          ...
          mainPath = (([
            "Main.hs"
          ] ++ pkgs.lib.optional (compiler.isGhc && compiler.version.lt "8.8") "") ++ pkgs.lib.optional (system.isAix) "") ++ pkgs.lib.optional (flags.git-rev) "";
        };
      };
      tests = {
        "unit-tests" = {

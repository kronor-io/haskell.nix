...
toNixPackageDescription isLocal detailLevel pd = mkNonRecSet $
    ...
    if detailLevel == MinimalDetails
      then []
        ...
        , "dataFiles"     $= toNix (fmap getSymbolicPath (dataFiles pd))
        , "extraSrcFiles" $= toNix (fmap getSymbolicPath (extraSrcFiles pd))
        , "extraTmpFiles" $= toNix (fmap getSymbolicPath (extraTmpFiles pd))
        , "extraDocFiles" $= toNix (fmap getSymbolicPath (extraDocFiles pd))
        ]
  where
          ...
          component unQualName comp
            = quoted name $=
                mkNonRecSet (
                  ...
                  [ "frameworks"   $= toNix deps | Just deps <- [shakeTree . fmap ( fmap (mkSysDep . getSymbolicPath) . frameworks . getBuildInfo) $ comp ] ] ++
                  ...
                  if detailLevel == MinimalDetails
                    then []
                      ...
                      [ "asmSources"   $= toNix src  | Just src  <- [shakeTree . fmap (fmap getSymbolicPath . asmSources   . getBuildInfo) $ comp ] ] ++
                      [ "cmmSources"   $= toNix src  | Just src  <- [shakeTree . fmap (fmap getSymbolicPath . cmmSources   . getBuildInfo) $ comp ] ] ++
                      [ "cSources"     $= toNix src  | Just src  <- [shakeTree . fmap (fmap getSymbolicPath . cSources     . getBuildInfo) $ comp ] ] ++
                      [ "cxxSources"   $= toNix src  | Just src  <- [shakeTree . fmap (fmap getSymbolicPath . cxxSources   . getBuildInfo) $ comp ] ] ++
                      [ "jsSources"    $= toNix src  | Just src  <- [shakeTree . fmap (fmap getSymbolicPath . jsSources    . getBuildInfo) $ comp ] ] ++
                      [ "hsSourceDirs" $= toNix (fmap getSymbolicPath <$> dir) | Just dir  <- [shakeTree . fmap (hsSourceDirs . getBuildInfo) $ comp ] ] ++
                      [ "includeDirs"  $= toNix dir  | Just dir  <- [shakeTree . fmap (fmap getSymbolicPath . includeDirs  . getBuildInfo) $ comp] ] ++
                      [ "includes"     $= toNix dir  | Just dir  <- [shakeTree . fmap (fmap getSymbolicPath . includes     . getBuildInfo) $ comp] ] ++

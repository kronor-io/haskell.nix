      ...
      (pkgName: subDir: rec {
        src =
          ...
          in if subDir == "libraries/ghc-boot"
            then nix24srcFix (final.buildPackages.runCommand "ghc-boot-src" { nativeBuildInputs = [(final.buildPackages.lndir or final.buildPackages.lndir)]; } ''
              mkdir $out
            ...
            then final.haskell-nix.haskellLib.cleanSourceWith {
              src = nix24srcFix (final.buildPackages.runCommand "ghc-src" { nativeBuildInputs = [(final.buildPackages.lndir or final.buildPackages.lndir)]; } ''
                mkdir $out
